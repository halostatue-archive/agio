...
describe Agio::Block do
  ...
  describe "should understand the elements represented by the Block" do
    describe "for HTML standard block element <p>" do
      subject { Agio::Block.new('p') }

      its(:name) { should == 'p' }
      its(:description) { subject.to_s.should == 'p: paragraph ' }

      its(:standard?) { should == true }
      its(:inline?) { should == false }
      its(:block?) { should == true }

      it "should not be able to contain another <p>" do
        subject.can_contain?(Agio::Block.new('p')).should_not == true
      end
